\chapter{Proof Normalization and Relationship to System \texorpdfstring{F$^\omega$}{F omega}}
...
System F$^\omega$ augmented with pairs is only slightly differently from the original PTS description of F$^\omega$.
...
\begin{enumerate}
    \item {
        Given $\varepsilon \vdash_{c_2} \abs{\lambda_0}{X}{\star}{
    ...
    \item {
        Given $\Gamma \vdash_{c_2} t : T$ where $\Gamma = A : \star;\ B : A_\square \to_\tau \star;\ a : A_\square;\ f : A_\square \to_\omega (x : A_\square) \cap B_\square$, $t = [(\app{f_\star}{\omega}{a_\star}).1, (\app{f_\star}{\omega}{a_\star}).2; (x : A_\square) \cap B_\square]$, and $T = (x : A_\square) \cap B_\square$ then
        \begin{align*}
            &\sema{A : \star; B : A \to_\tau \star; a : A; f : A \to_\omega (x : A) \cap B} =\\
            &\quad 0 : \star;\ \bot : (X : \star) \to X;\ A : \star;\ w_A : 0;\ B : \star;\ w_B : A \to 0; \\
            &\quad a : A;\ f : A \to A \times B
        \end{align*}
        \begin{align*}
            &[(\app{f}{\omega}{a}).1, (\app{f}{\omega}{a}).2] =\\
            &\quad (\abs{\lambda}{y}{0}{
                ((f\ a).1, (f\ a).2)
            })\ (c^{0 \to 0 \to 0}\ w_A\ [x := c^A]w_B)
        \end{align*}
        $$(x : A) \cap B = A \times B$$
    }
\end{enumerate}
...
\begin{theorem}[Soundness of $V$]
    ...
    If $\Gamma \vdash_{c_2} t : \kind$ then $\Delta \vdash_\omega V(t) : \kind$ for any $\Delta$
\end{theorem}
\begin{proof}
    By induction on $\Gamma \vdash_{c_2} t : \kind$.
...
\begin{theorem}[Soundness of $\sema{-}$]
    ...
    If $\Gamma \vdash_{c_2} t : A$ then $\sema{\Gamma} \vdash_\omega \sema{t}_\Gamma : V(A)$
    where $A = \kind$ or $\Gamma \vdash A : \kind$
\end{theorem}
\begin{proof}
    By induction on $\Gamma \vdash_{c_2} t : A$.
    ...
    $\text{Case: }\begin{array}{c} \AxiomRule[*] \end{array}$
    \begin{proofcase}
        By computation $\sema{\star}_\Gamma = 0$ and $V(\kind) = \star$.
        Note that $0 : \star \in \sema{\Gamma}$ thus this case is concluded by the \textsc{Var} rule.
    \end{proofcase}

    $\text{Case: }\begin{array}{c} \VarRule[*] \end{array}$
    \begin{proofcase}
        By computation $\sema{x}_\Gamma = x$.
    ...
    \begin{proofcase}
        ...
        \begin{enumerate}
            \item[$\D{1}$.] $\sema{\Gamma} \vdash_\omega \sema{A}_\Gamma : \star$
            \item[$\D{2}$.] $\sema{\Gamma, x_m : A} \vdash_\omega \sema{B}_{\Gamma, x:A} : \star$ 
        \end{enumerate}
        Suppose that $A$ is a kind.
        Then $\sema{(x : A) \to_m B}_\Gamma = (x : V(A)) \to \sema{A}_\Gamma \to \sema{B}_{\Gamma,x:A}$ and $\sema{\Gamma, x_m : A} = \sema{\Gamma}, x : V(A), w_x : \sema{A}_\Gamma$.\
        The \textsc{Pi2} rule applied with the results of the IH gives
            \begin{tightcenter} $\sema{\Gamma}, x : V(A) \vdash_\omega \sema{A}_\Gamma \to \sema{B}_{\Gamma, x : A} : \star$ \end{tightcenter}
        Now by Lemma~\ref{lem:3:soudness_of_v} applied to $\D{1}$: $\sema{\Gamma} \vdash_\omega V(A) : \kind$.
        Using the \textsc{Pi1} rule gives $\sema{\Gamma} \vdash_\omega V(A) \to \sema{A}_\Gamma \to \sema{B}_{\Gamma, x : A} : \star$.
        \\ \\
        Suppose that $A$ is a type.
        Then $\sema{(x : A) \to_m B}_\Gamma = (x : \sema{A}_\Gamma) \to \sema{B}_{\Gamma,x:A}$ and $\sema{\Gamma, x_m : A} = \sema{\Gamma}, x : \sema{A}_\Gamma$.
        Thus, by the \textsc{Pi2} rule $\sema{\Gamma} \vdash \sema{A}_\Gamma \to \sema{B}_{\Gamma,x:A} : \star$.
    \end{proofcase}
    ...
    \begin{proofcase}
        ...
        Applying the IH gives:
        \begin{enumerate}
            \item[$\D{1}$.] $\sema{\Gamma} \vdash_\omega \sema{(x : A) \to_\tau B}_\Gamma : \star$
            \item[$\D{2}$.] $\sema{\Gamma, x_\tau : A} \vdash_\omega \sema{t}_{\Gamma, x:A} : V(B)$ 
        \end{enumerate}
        Suppose $A$ is a kind.
        Then $\sema{(x : A) \to_\tau B}_\Gamma = (x : V(A)) \to \sema{A}_\Gamma \to \sema{B}_{\Gamma,x:A}$, $\sema{\Gamma, x_m : A} = \sema{\Gamma}, x : V(A), w_x : \sema{A}_\Gamma$, and $\sema{\abs{\lambda_\tau}{x}{A}{t}}_\Gamma = \abs{\lambda}{x}{V(A)}{\sema{t}_{\Gamma,x:A}}$.
        Note that $\sema{\Gamma} \vdash c^{\sema{A}_\Gamma} : \sema{A}_\Gamma$.
        Thus, by substitution lemma for F$^\omega$: $\sema{\Gamma}, x : V(A) \vdash_\omega [w_x := c^{\sema{A}_\Gamma}]\sema{t}_{\Gamma, x:A} : [w_x := c^{\sema{A}_\Gamma}]V(B)$.
        However, because $A$ is kind and by Lemma~\ref{lem:3:var_not_in_sema_when_kind}: $[w_x := c^{\sema{A}_\Gamma}]\sema{t}_{\Gamma, x:A} = \sema{t}_{\Gamma, x:A}$.
        Note also that $FV(V(B))$ is empty, thus $[w_x := c^{\sema{A}_\Gamma}]V(B) = V(B)$.
        ...
        \\ \\
        Suppose $A$ is a type.
        Then $\sema{\Gamma, x_m : A} = \sema{\Gamma}, x : \sema{A}_\Gamma$ and $\sema{\abs{\lambda_\tau}{x}{A}{t}}_\Gamma = \sema{t}_{\Gamma,x:A}$.
    ...
    \begin{proofcase}
        ...
        Applying the IH to $\D{1}$ thus gives $\sema{\Gamma} \vdash_\omega \sema{f}_\Gamma : V((x : A) \to_\tau B)$.
        \\ \\
        Suppose $A$ is a kind, then $a$ is a type.
        Thus, $V((x : A) \to_\tau B) = V(A) \to V(B)$ and $\sema{\app{f}{\tau}{a}}_\Gamma = \sema{f}_\Gamma\ \sema{a}_\Gamma$.
        Applying the IH to $\D{2}$ gives $\sema{\Gamma} \vdash_\omega \sema{a}_\Gamma : V(A)$.
        By the \textsc{App} rule: $\sema{\Gamma} \vdash \sema{f}_\Gamma\ \sema{a}_\Gamma : V(B)$.
        Now by Lemma~\ref{lem:3:v_subst}: $V(B) = V([x := a]B)$.
        \\ \\
        Suppose $A$ is a type, then $a$ is a term.
        Thus, $V((x : A) \to_\tau B) = V(B)$ and $\sema{\app{f}{\tau}{a}}_\Gamma = \sema{f}_\Gamma$.
        But, $\sema{\Gamma} \vdash_\omega \sema{f}_\Gamma : V(B)$ already.
        Now by Lemma~\ref{lem:3:v_subst}: $V(B) = V([x := a]B)$.
    \end{proofcase}
    ...
    \begin{proofcase}
        ...
        \end{enumerate}
        Note that $A$ is a type thus $\sema{\Gamma, x_\tau : A} = \sema{\Gamma}, x : \sema{A}_\Gamma$.
        Applying the \textsc{Lam} rule twice reduces the goal to $\sema{\Gamma}, \sema{A}_\Gamma : \star, \sema{B}_{\Gamma,x:A} : \star \vdash_\omega \sema{A}_\Gamma \times \sema{B}_{\Gamma,x:A} : \star$.
    ...
    \begin{proofcase}
        By computation $\sema{a =_A b}_\Gamma = \fpId$ and $V(\star) = \star$.
    ...
    \begin{proofcase}
        ...
        Applying the IH to $\D{2}$ gives $\sema{\Gamma} \vdash_\omega \sema{t}_\Gamma : V(B)$.
...
First, classification of syntax in $c_2$ is shown to be equal wrt erasure if the resulting kind semantics is well-defined.
This is then lifted to show that erasure has no impact on the semantics of kinds, again assuming the kind semantics is well-defined.
Next, classification of syntax in $c_2$ is shown to be equal wrt erasure if the resulting type semantics is well-defined.
And, again, this result is lifted to show that erasure has no impact on the semantics of types.

\begin{lemma}
    \label{lem:3:vsema_erase_lemma}
    If $\Gamma \vdash V(t) : \kind$ then $\mathcal{C}(t) = \mathcal{C}(|t|)$
\end{lemma}
\begin{proof}
    By induction on $t$ and inversion on $\Gamma \vdash V(t) : \kind$.
    ...
    $\text{Case: } t = \star \text{ or } t = \kind$
    \begin{proofcase}
        Note that $|\!\star\!| = \star$ and $|\kind| = \kind$, thus this case is trivial.
    \end{proofcase}

    $\text{Case: } t = (x : A) \to_m B$
    \begin{proofcase}
        Have $|(x : A) \to_m B| = (x : |A|) \to_m |B|$.
        By computation $\mathcal{C}((x : A) \to_m B) = \mathcal{C}((x : |A|) \to_m |B|)$.
    \end{proofcase}
\end{proof}

\begin{lemma}
    \label{lem:3:vsema_erase}
    If $\Gamma \vdash V(t) : \kind$ then $V(t) = V(|t|)$
\end{lemma}
\begin{proof}
    By induction on $t$ and inversion on $\Gamma \vdash V(t) : \kind$.
    ...
    \begin{proofcase}
        Suppose $A$ is a kind.
        By Lemma~\ref{lem:3:vsema_erase_lemma}: $|A|$ kind.
...
\begin{lemma}
    \label{lem:3:sema_erase_lemma}
    If $\Gamma \vdash \sema{t} : T$ then $\mathcal{C}(t) = \mathcal{C}(|t|)$
\end{lemma}
\begin{proof}
    By induction on $t$ and inversion on $\Gamma \vdash \sema{t} : T$.
    For every case erasure is homomorphic and $\mathcal{C}(t)$ considers only the immediate tag.
    Except the base cases, the rest are very similar.
    Thus, only the eq case is considered.
    

    $\text{Case: } t = \star \text{ or } t = \kind \text{ or } t = x_\square$
    \begin{proofcase}
        For all cases $|t| = t$, thus trivial.
    \end{proofcase}

    $\text{Case: } t = a =_A b$
    \begin{proofcase}
        Have $|a =_A b| = |a| =_{|A|} |b|$ but $\mathcal{C}(|a| =_{|A|} |b|) = \ptype$ for any subexpressions.
    \end{proofcase}
\end{proof}

\begin{lemma}
    \label{lem:3:sema_erase}
    If $\Gamma \vdash \sema{t} : T$ then $\sema{t} = \sema{|t|}$
\end{lemma}
\begin{proof}
    By induction on $t$ and inversion on $\Gamma \vdash \sema{t} : T$.
...
\begin{lemma}
    ...
    If $\Gamma \vdash V(s) : \kind$ then $|s| \betared t$ then $V(s) = V(t)$
\end{lemma}
\begin{proof}
...
\begin{lemma}
    ...
    If $\Gamma \vdash \sema{s} : T$ and $|s| \betared t$ then $\sema{s} \betared \sema{t}$ or $\sema{s} = \sema{t}$
\end{lemma}
\begin{proof}
    By induction on $|s| \betared t$.
...
\begin{lemma}
    ...
    If $\Gamma \vdash \sema{s} : T$ and $|s| \betastar t$ then $\sema{s} \betastar \sema{t}$
\end{lemma}
\begin{proof}
...
Finally, soundness of the term semantics is proven by a straightforward induction on the inference judgment of $c_2$.

\begin{lemma}
    \label{lem:3:sema_conv}
    If $\Gamma \vdash \sema{A} : T$, $\Gamma \vdash \sema{B} : T$, $A, B$\pseobj, and $A \equiv B$ then $\sema{A} \betaconv \sema{B}$
\end{lemma}
\begin{proof}
...
\begin{theorem}[Soundness of \text{[}$-$\text{]}]
    ...
    If $\Gamma \vdash_{c_2} t : A$ then $\sema{\Gamma} \vdash_\omega [t] : \sema{A}$
\end{theorem}
\begin{proof}
    By induction on $\Gamma \vdash_{c_2} t : A$.
    ...
    \begin{proofcase}
        ...
        \begin{enumerate}
            \item[$\D{1}.$] $\sema{\Gamma} \vdash_\omega [A] : 0$
            \item[$\D{2}.$] $\sema{\Gamma} \vdash_\omega [e] : \fpId$
            \item[$\D{3}.$] $\sema{\Gamma} \vdash_\omega [P] : \sema{A} \to \fpId \to 0$
        \end{enumerate}
        ...
        Applying Lemma~\ref{lem:3:fomega_eta} twice concludes the case.
    \end{proofcase}
    ...
    \begin{proofcase}
        By computation $[\delta(e)] = (\abs{\lambda}{x}{\mathcal{I}([e]_\Gamma)}{\bot})\ [e]_\Gamma$ and $\sema{(X : \star) \to_0 X} = (X : \star) \to X$.
        Note that $\sema{\Gamma} \vdash_\omega \bot : (X : \star) \to X$ and by definition $\sema{\Gamma} \vdash_\omega [e]_\Gamma : \mathcal{I}([e]_\Gamma)$.
        Thus, by Lemma~\ref{lem:3:fomega_eta}: $\sema{\Gamma} \vdash [\delta(e)] : \sema{(X : \star) \to_0 X}_\Gamma$.
    \end{proofcase}

    $\text{Case: }\begin{array}{c} \ConvRule[*] \end{array}$
    \begin{proofcase}
        By classification, $\D{1}$ and $\D{3}$: $\Gamma \vdash B : K$.
        Now using Theorem~\ref{lem:3:soundness_of_sema} gives $\sema{\Gamma} \vdash_\omega \sema{A}_\Gamma : \star$ and $\sema{\Gamma} \vdash_\omega \sema{B}_\Gamma : \star$.
...
That is, proof reduction in $c_2$ is bounded by reduction in F$^\omega$, and thus because F$^\omega$ is strongly normalizing it provides a maximum number of reduction steps for which any proof must normalize in $c_2$.
...
\begin{proof}
    ...
    $\text{Case: }\begin{array}{c} \app{\psi(\text{refl}(t; A_1); A_2, P)}{\omega}{t} \betared t \end{array}$
    \begin{proofcase}
        Note that $t$ is a term by inversion on $\Gamma \vdash s : T$.
        Have $[\app{\psi(\text{refl}(t; A_1); A_2, P)}{\omega}{t}] =
            (\abs{\lambda}{y_1}{0}{
                \abs{\lambda}{y_2}{\sema{A_2} \to \fpId \to 0}{
                    [\text{refl}(t; A_1)]\ \sema{P}
                }
            })\ [A_2]\ [P]\ [t]$.
        Applying two $\beta$-reductions yields $[\text{refl}(t; A_1)]\ \sema{P}\ [t]$.
        Now $[\text{refl}(t; A_1)] =
            (\abs{\lambda}{y_1}{0}{
