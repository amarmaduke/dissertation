...
\begin{proof}
    ...
    \begin{proofcase}
        ...
        Applying the IH to subderivations:
        \begin{enumerate}
            \item[$\D{2}.$] $\sema{\Gamma} \vdash_\omega [(x : A) \cap B] : 0$
            \item[$\D{3}.$] $\sema{\Gamma} \vdash_\omega [a] : \sema{A}$
            \item[$\D{4}.$] $\sema{\Gamma} \vdash_\omega [b] : \sema{(x : A) \cap B}$
            \item[$\D{4}.$] $\sema{\Gamma} \vdash_\omega [e] : \fpId$
        \end{enumerate}
        Deconstructing $\sema{\Gamma} \vdash_\omega [(x : A) \cap B] : 0$ gives $\sema{\Gamma} \vdash_\omega [A] : 0$.
        By Lemma~\ref{lem:3:canonical_element}: $\sema{\Gamma} \vdash_\omega c^{\sema{T}} : \sema{T}$.
        Applying Lemma~\ref{lem:3:fomega_eta} five times concludes the case.
    \end{proofcase}
