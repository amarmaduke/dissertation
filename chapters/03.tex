...
Proof normalization is a weak property, because it does not entail consistency.
...
The kind and type semantics are allowed to discard reductions because both only serves the purpose of maintaining a well-typed output.
Instead, the term semantics must take care to preserve all possible reductions such that strong proof normalization is a consequence of the model.

For dependent intersections, the type semantics is: $\sema{(x : A) \cap B} = \sema{A} \times \sema{B}$.
...
\begin{proof}
    ...
    \begin{proofcase}
        By $\D{1}$: $\Gamma \vdash \kind : K$ which is impossible.
    \end{proofcase}
...
This is not like the original argument for CC modelled in F$^\omega$ because conversion is relative to erasure.
...
For kinds this turns out to be equality, as any possible dependence involving reduction are always dropped the structure of $V(t)$ for any $t$ is rigid.
...
\begin{proof}
    ...
    If $\sema{s} = \sema{z}$ then $\Gamma \vdash \sema{z} : T$ and the same argument as above works.
\end{proof}
...
\begin{proof}
    ...
    Note that $[-]$ replicates every subexpression on the left-hand side with a matching invocation of $[-]$ on the right-hand side.
