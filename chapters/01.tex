...
For the unfamiliar reading this rules is arcane, thus below is exposition of a selection of the typing rules to better acclimate the reader with the notation:
...
$\VarRule$ The variable rule is a context lookup.
...
Let $a = \mathbb{N}$ (any type constant), then $f\ a \infr [X := \mathbb{N}](X \to X)$ or $f\ a \infr \mathbb{N} \to \mathbb{N}$.
...
Thus, the standard metatheoretical results, such that well-typed terms of F$^\omega$ are strongly normalizing still holds.
...
$$\neg A = A \to \bot$$
These definitions are called \textit{Church encodings} and originate from Church's initial encodings of data in the $\lambda$-calculus.
However, recall that induction principles cannot be derived internally for these definitions \cite{geuvers2001_noind}.
