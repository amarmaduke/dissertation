...
Notice that $|a| = |b|$ but $|b| \neq |c|$, however, $c \betastar x$, thus $b \equiv c$ and $\neg (b \equiv c)$.
There is an inconsistency in the definition because $b$ is not a pseudo-object, it is not the case that $|x| \betaconv |y|$.
Really this is more fundamental than just transitivity as it shows that reduction is not consistent with erasure unless the syntax is a pseudo-object.
...
Having Theorem~\ref{thm:2:beta_iff_conv} means that whenever conversion is checked between terms it is safe to instead check convertibility of reduction of objects.
